...
-- CHAIN-04's decoder, imported for its TYPES and its SIGNATURE STRING as well as its function.
-- 'shock_signature' is hashed by the check below rather than transcribed a second time, so a drift
-- between the library's idea of the event and the test's is a failure instead of two copies each
-- agreeing with itself. Nothing here touches a chain: 'decode_shock' is a total function from a
-- 'Change' this file builds to an 'Either'.
import Chain.Shock
  ( ShockDecodeError (..)
  , ShockEvent (..)
  , decode_shock
...
ground_truth =
  ...
  , ( "topic0"
    ...
    -- CHAIN-04's event. It is pinned HERE and deliberately NOT in 'expected_topic_pins', and the
    -- reason is structural rather than an oversight: the pin surface is generated by
    -- @offchain\/rig\/generate-pins.sh@, which iterates the interface files present in THIS
    -- worktree, and this event's emitter
    -- (@src\/models\/mev_tax_model_one\/libraries\/ShockLib.plk@) is not one of them. It lives on
    -- @origin\/develop@, merged there by PR #30, and this branch is 84 ahead \/ 293 behind and must
    -- not be merged for this phase. Adding the name to that set would redden
    -- 'sc4_pin_surface_is_the_expected_set' immediately, in its both-directions arm, because the
    -- generator would never produce it.
    --
    -- MEASURED with @cast keccak@ on 2026-08-17, and CONFIRMED against the emitter's own
    -- hand-written @SHOCK_EVENT_TOPIC0@ constant read out of @origin\/develop@ with @git show@:
    -- the two are the same 32 bytes. 'the_upstream_shocklib_pin_is_a_live_trip_wire' is what will
    -- say when the pin idiom becomes available again -- and its failure text asks for the CONSTANT
    -- to be re-verified, not merely for the pin to be re-homed.
  , ( "topic0"
    , "Shock(address,int24,uint24,uint24)"
    , "21b0e4f81f5ef89be4325ca74966f2fb8f57a217e284dd3e0a276fff55987d64"
    )
  ]
...
shock_topic0_is_computed_from_the_signature_string :: Check
shock_topic0_is_computed_from_the_signature_string =
  pure_check "shock_topic0_is_computed_from_the_signature_string" $ do
    (sig, want) <- truth_for "Shock"
    _ <- expect (sig == shock_signature)
           ("the ground-truth row's signature is " ++ show sig ++ " and the LIBRARY's"
    ...
    _ <- expect (to_hex (topic0_of sig) == want)
           ("keccak of " ++ show sig ++ " is " ++ to_hex (topic0_of sig)
             ++ ", and the pinned ground truth measured with cast is " ++ want)
