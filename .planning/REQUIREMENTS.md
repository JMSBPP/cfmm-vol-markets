...
| LOOP-02 | Phase 28 | **In progress (28-01) — and the "Blocked" attribution was INHERITED, not measured.** Both of LOOP-02's directions are now asserted CHAIN-FREE, against `Store.Memory` + `new_memory_ledger` + a counting stub solver: `the_same_event_twice_produces_one_row_and_no_second_solve` (one row, one solve, one store entry, with the first pass asserted to have happened BEFORE any absence is read) and `two_distinct_events_with_one_shock_make_one_entry_and_two_rows` (two rows, ONE entry, ONE solve, tokens `stored` then `elided`). Neither needs an emitter — the requirement is about the ledger and the key, not about the chain — which is the same correction 27 made for CHAIN-02/03, whose Blocked status came from CHAIN-01's row rather than from a measurement. **STILL OPEN:** the requirement is stated over the LOOP, and there is no loop yet; 28-02 writes it. **MEASURED FINDING (28-01 M1):** dropping the `ledger_seen` guard does NOT cost a second solve — the content key still elides it — so the ledger guard and the content key are not two spellings of one mechanism, and what the guard buys is the pipeline doing NOTHING on a replay. |
