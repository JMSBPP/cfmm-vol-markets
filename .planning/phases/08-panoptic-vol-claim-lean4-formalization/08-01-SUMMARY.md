...
provides:
  - "Pinned, git-tracked spec/panoptic.md with corrected negative-exponent θ kernel"
  - "URL/citekey Demeterfi (1999) volatility-swaps reference (no vendored PDF)"
  - "Vendored load-bearing cfmm-discrete proof-source notes under spec/refs/cfmm-discrete/"
  - "spec/refs/README.md provenance note (cfmm-theory KB citekey remains canonical)"
...
**Pinned spec/panoptic.md with the corrected negative-exponent θ kernel and URL/citekey Demeterfi reference, and vendored the six load-bearing cfmm-discrete proof-source notes in-tree with dangling cross-tree links neutralized.**
...
- Committed the previously-untracked `spec/panoptic.md` as the phase's pinned source of truth.
- Vendored six cfmm-discrete notes (FINANCE, STREAMING_PREMIUM, DIFFERENTIATION, BINARY_TREES, INTEGRATION, COORDINATES) into `spec/refs/cfmm-discrete/`, neutralizing cross-tree links to `../lp-derivatives/*` and `../cfmm-options/*` while keeping in-tree sibling links live.
- Added `spec/refs/README.md` provenance note.

## Task Commits

Each task was committed atomically:

1. **Task 1: Fix θ sign, Demeterfi citekey, and ~/ NOTE path in spec/panoptic.md; commit** — `4316306` (docs)
...
- `spec/panoptic.md` — pinned/tracked; negative-exponent θ kernel, URL/citekey Demeterfi ref, de-pathed NOTE
- `spec/refs/cfmm-discrete/FINANCE.md` — CRR backward-induction operator + σ-bridge source
- `spec/refs/cfmm-discrete/STREAMING_PREMIUM.md` — lattice θ_ATM derivation (center-column, kσ/√(8πτ))
- `spec/refs/cfmm-discrete/DIFFERENTIATION.md` — discrete Itô ∂_t/∂_i² operator
- `spec/refs/cfmm-discrete/BINARY_TREES.md` — Δt=(Δi)² heat-operator collapse, geometric-grid ⟹ CRR
- `spec/refs/cfmm-discrete/INTEGRATION.md` — base forms/incidence-weight primitives (keeps ./INTEGRATION.md links live)
- `spec/refs/cfmm-discrete/COORDINATES.md` — reserve 0-form / trading-flow lattice calculus
- `spec/refs/README.md` — provenance note; cfmm-theory KB citekey remains canonical
...
- **Issue:** The first draft of `spec/refs/README.md` used the literal example strings `~/`, `$HOME`, and `/home/...` when explaining the Phase-1 sanitization rule. The plan's own acceptance check `grep -rnE '/home/|\$HOME|~/' spec/refs/` cannot distinguish an explanatory mention from a real path, so the draft failed the "no home-absolute paths" gate (exit 0 instead of 1).
...
- **Files modified:** spec/refs/README.md
- **Verification:** `grep -rnE '/home/|\$HOME|~/' spec/refs/` now returns nothing (exit 1).
...
- `spec/panoptic.md` is pinned, tracked, and sign-corrected — downstream Lean plans (08-02+) can reference it as the frozen source of truth.
- The load-bearing lattice calculus notes are in-tree at `spec/refs/cfmm-discrete/`; the Aristotle θ_ATM(τ)=kσ/√(8πτ) derivation (the phase's heaviest proof) can cite STREAMING_PREMIUM.md and FINANCE.md via stable in-tree paths.
