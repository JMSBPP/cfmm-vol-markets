...
| **Aggregate gate cost — stated, not hidden** | The 900 s ceiling is **per run**. **CORRECTED 2026-08-17 — this row previously claimed "every one of the 28 tasks gates on a full `cabal test`". That is FALSE. MEASURED: 16 of 28 tasks carry a `cabal test` gate; the other 12 gate on `cabal build --enable-tests` only** — 25-01 T1, 25-02 T1, 25-03 T1, 25-04 T1+T2, 25-05 T1+T2+T3, 25-06 T1, 25-07 T1, 25-08 T1, 25-09 T2. `cabal test` starts at 152.9 s and rises toward ~200 s at 207 checks, so the real gate cost is **~45–55 minutes**, not the 80–90 the old figure implied, before the six Tier-C tasks' Docker capture. **The coarser rate is not a saving — it is a defect mechanism.** A build-only task can leave the suite RED and the redness is not discovered until the NEXT task's gate; the phase-25 review found exactly that shape twice (25-01 T1→T2 on `aeson_storage_path`, 25-08 T1→T2 on `verifier_importers`). An executor must therefore treat a build-only task as **unverified**, not as passed, and must not defer a known-red state across a task boundary. The trade is accepted — one file and one runner means there is no cheaper subset — but it is recorded here as a cost with a failure mode, not as a clean sampling rate |
...
- **Prose is inside a grep's blast radius** — eighteen-plus instances across phases 23–25. A
  ...
  `Main.hs:7842`), and the fragment joins are themselves asserted through a shared positive control.
