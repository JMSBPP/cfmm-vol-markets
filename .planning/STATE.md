...
- **OPEN QUESTION — `[rpc_endpoints]` `${...}` interpolation timing.** The bridge publishes `EVM_SPEC_BRIDGE_URL`, consumed via an `evm_spec_bridge` alias. No name collision with the existing `mainnet`/`local` keys. But if Foundry resolves `${EVM_SPEC_BRIDGE_URL}` at **config load** rather than at alias *use*, an unset variable would fail the whole config — taking down `forge build` and all ~250 unrelated tests, presenting as a total gate collapse with an unguessable cause. Unverified; do NOT assert either way. Mitigation already available: the literal-URL form works with no `foundry.toml` entry at all, so the alias is optional.
