...
| **PR-ORIENT** | `χ_{X/M}` leg orientation. Two displays 32 lines apart put it on opposite legs; Theorem 1 consumes one, Lean `phiCES` matches the other. Flips the `χ/(1−χ) = λ^{ηΔ_i/2}` bridge and the doc reading of `curvIndex_is_rho_zero_slice`. | **CC-REPL**, **CC-CURV** | author decision | Ph 13 (f) | **OPEN — flagged in doc** |
...
| **PR-GATE** | `eta-notation-gate.sh` enforces the **old** κ_φ bindings and now fails on the corrected doc. It is the house precondition for landing any doc block. | **every claim with a pending doc block** — CC-IV, CC-GREEK, CC-OCC, CTX-DEFORDER | tooling | Ph 13 (c) | **OPEN — critical path** |
| **PR-EMBED** | Embedding-test verdict `232c8ee4`: does Capponi's convex-combination family embed in full CES? Decides whether E4 is redone on the ε axis or closed MOOT. | **CC-CURV** | external | Ph 13 (a) | **BLOCKED — Aristotle API 500** |
...
| **PR-EPSTOL** | A bare unsubscripted `ε` survives as a tolerance, against the binding rule that ε is reserved for elasticities and always subscripted. | notation hygiene only | notation | Ph 13 (h) | OPEN — MINOR |
