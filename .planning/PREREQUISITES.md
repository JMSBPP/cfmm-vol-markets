...
| **CC-OCC** | The occupancy fraction `T_ITM/T` connects to our endogenous `T` | `## OTHER KRSITENSEN CONNECTIONS` | ✗ **CLOSED 2026-08-03 — REFUTED AS POSED.** Kristensen's `T` is an exogenous user-chosen duration over a position his own text says *never expires*; ours is an endogenous output. Redirect: the object belongs on the FLAIR / `Γ^Σ` side, where the band indicator already lives |
...
| **PR-OCCOBJ** | Does Kristensen's `T` denote a maturity at all? | **CC-OCC** | research | spike | ✓ **ANSWERED 2026-08-03: NO.** p. 55 — the position *never expires*; p. 58 — `T` is a user-controlled holding duration. Spike CLOSED, do not promote |
