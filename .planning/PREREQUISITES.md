...
| **CC-OCC** | The occupancy fraction `T_ITM/T` connects to our endogenous `T` | `## OTHER KRSITENSEN CONNECTIONS` | ◐ **ROUND 2 DONE 2026-08-03 — PROMOTE narrowly, conditionally.** Inception reading (`T★` fixed at mint) well-posed; running reading ill-posed without a stopping rule (user decision). Provable today with no new axioms: the Panoptic predicate algebra + the abstract occupancy layer + **in-band fraction strictly decreasing in target vega** (composes the proved `tStar_strictMono_dQvStar`, verified at `EndogenousMaturity.lean:95`). Explicitly NOT promoted: `ℙ_ITM = 𝔼[𝟙]` (no probability space in the tree, no measure convention, which-tick unresolved), any closed form for the integrand, the running-`T★` fixed point. R4 stands: evaluate an occupancy functional AT `T★`, never multiply `T★` BY one. Four user rulings owed. Previously: REOPENED after the exogenous/endogenous close-out was refuted by the user. The close-out rested on "his `T` exogenous, ours endogenous" — **false**: `T★ = 2ΔQ_v★/N_σ` and `ΔQ_v★` is a first-class on-chain order field, so ours is user-controlled too, and both are equivalence horizons converting a perpetual into a dated instrument. What survives: `T_ITM/T` is ruled out as a **modifier of the maturity law** (R4 — a band weight is finite-strip replication error). What is OPEN: the **composition** the user actually asked for — with `T = T★`, what is `T_ITM/T★` and what does it buy |
...
| **PR-THETA** | The θ exponent sign. | **CC-GREEK**, CTX-DEFORDER | author decision | Ph 12.1 | ✓ **RULED 2026-08-03: NEGATIVE** (plank `a48e1ea`). Justification recorded in-doc: exact r=0 BS dt-leg match via \(S\varphi(d_1)=K\varphi(d_2)\), and — decisive — \(p_{call}\leftarrow\int\theta\) converges only under the negative sign. Unblocks the θ Definition (12.1), G1's `θ_decay` + the on-chain constant (Ph 15) |
...
| **PR-SYNC** | `VOLATILITY_INSTRUMENTS_ETA_ADDENDUM.md` desynced from the plank copy (155 stale `\kappa_{\varphi` sites, 0 `\varsigma`), and its `APPROVED-ETA-SHA256` disclosure is itself stale. Re-pinning is required before any **new** bundle is submitted against that section. | CC-CURV (re-submission only) | integrity | Ph 13 (d) | ✓ **DONE 2026-08-03** |
| **PR-CSYM** | A **free** symbol pair for the replication weights. | CTX-DEFORDER | notation | Ph 12.1 | ✓ **SETTLED 2026-08-03: `a₁, a₂`** — free (the two grep hits were substrings of `\alpha_1`/prose), and exactly the Lean carrier names (`replicationPrice p0 a1 a2 …`); shown in the pair session, no objection. Lands with the replication statement when its dependency chain completes |
...
| **PR-OCCOBJ** | Does Kristensen's `T` denote a maturity at all? | **CC-OCC** | research | spike | ◐ **SUPERSEDED as the gating question.** Answered (his `T` is a holding duration, p. 55/58) but the answer does not close CC-OCC: ours is user-controlled too. The gating question is now whether integrating occupancy over an **implied maturity that is never held** is meaningful |
