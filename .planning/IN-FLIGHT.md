...
| `70b9558f-b62d-40c1-9b39-87c2f8fc73ea` | `aristotle-pi` | 2026-08-11 | **6 sorry'd targets in `PiPayoffs.lean`** (`scratch/pi-submit/`, self-contained): the ADJUSTED π family (TODO item 3, landed as doc Defs 42–44 + Prop 15, plank `638adfa`) — carriers AND consistency checks against the proved layer. **P1** π^log gamma = −1/P² + the K^{−1/2} law (CLOSES Prop 6's premise, its last OPEN half) + π^f zero-gamma; **P2** smooth-path LVR ≡ 0 (the boxed law ∘ envelope — LVR is pure quadratic variation); **P3** the discrete second-order step, littleO form — the lattice grounding of the user's d⟨P⟩ FLAG as (ΔP)² through ½Γ; **P4** HODL tangent-drift consistency (Def 30 vs Def 42); **P5** IL = 0 at inception; **P6** IL ≥ 0 (tangent dominates the concave value; refute-and-correct invited on the antitone hypothesis). Any refutation IS the sought inconsistency, per the user's instruction. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | usual gates; on return promote Prop 15, close Prop 6's premise (Defs 34/35 gain carriers), and record any refutation as the found inconsistency in-statement. STILL OWED (user rulings): π^φ as function of flow (TODO item 2 partials); Definition 14 ε_{p/X} bare-Q_X. |

| `676a5787-e5b5-4a01-a682-52a6c0faecd3` | `aristotle-kappa-coord` | 2026-08-11 | **4 sorry'd targets in `KappaCoordinate.lean`** (`scratch/kappa-coord-submit/`, self-contained): the COORDINATE ATLAS — the user's structured proposal. **N1** normalization (the pure gamma coordinate is the RATIO Γ_φ(i)/Γ_φ(i_0) = ξ^{−3η(i−i_0)}; liquidity cancels — the user's units fix, already landed as Def 41's normalization clause, plank `42675ab`); **N2** primal-dual reciprocity (gamma map × impact map = 1 — the (Q_X^L, p_φ) conjugate pair); **N3** the geometric ladder's κ-map (log-slope of liquidity against marginal price) is CONSTANT 1/(2ηΔ_i), = 3/2 at the flatness threshold ηΔ_i = 1/3 — the gamma shape's own exponent; **N4 THE CLAIM (iff)**: the κ-map is tick-constant IFF the density is geometric — off the geometric family it is a GENUINE coordinate; the trading/curvature axis gets its coordinate from the BOOK, not the member (κ_φ being tick-constant on CES). Refute-clause pointed at N4's reverse direction. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | usual gates; on return land the atlas statements (κ-map definition + N3/N4) next to Def 41, and cross-tie N4 to Prop 12 (both are geometric-iff verdicts — one on the field, one on the κ-map). |
