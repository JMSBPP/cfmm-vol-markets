...
|---|---|---|---|---|---|
| `7d9a8baa-1c40-49fb-9bcf-708b71dc3297` | `aristotle-phimix` | 2026-08-04 | **4 sorry'd targets in `PhiMix.lean`** (`scratch/phimix-submit/`): M3 THE TARGET — the anchor's curvature family in φ-terms, `F_ς = (1−ς)A(p+1)φ_(p/(p+1),1) + ς(φ_(1/2,0))²` (eliminates F per the user constraint "we CANNOT introduce F"); M1 linear component = rescaled ε=1 member; M2 constant-product = squared balanced member; M4 homogeneity obstruction iff (refute-and-correct clause). Priority M3>M1>M2>M4. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | **Do not integrate blind**: gates as usual; on success land in E1 as the numbered THEOREM (user pre-ruling: "not a proposition but a theorem or a protocol rule") replacing the F-display with the φ-decomposition — E1's family display rewritten so F never enters the doc's object language. |
