...
| `284fb7da-5784-493b-9ab9-631b32657980` | `aristotle-kappa-struct` | 2026-08-11 | **3 sorry'd targets in `KappaStructure.lean`** (`scratch/kappa-struct-submit/`, imports proved EllIntrinsic + CanonicalParam + PayoffGeometry): WHAT κ_φ STRUCTURALLY IS — **S1** the metric-free part of the level-curve planar curvature IS the price impact (strip (1+p²)^{3/2}, the factor needing the metric reserve space lacks); **S2** the elasticity = the metric-free curvature dressed by the state ratio x/p; **S3** the benchmark ratio recovers Definition 14's (1−ε)/(2−ε) at balanced reserves — member-against-member in place of the unavailable metric. Doc side already landed as Definition 14's STRUCTURAL TYPING note (`4381cf3`). | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | usual gates; on return attach the carriers to Definition 14's typing note. |
