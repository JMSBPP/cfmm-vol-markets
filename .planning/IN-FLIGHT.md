...
| `d1ad6474-7b98-4d19-baee-f9c63fd1e86a` | `aristotle-making` | 2026-08-11 | **8 sorry'd targets in `MarketMaking.lean`** (`scratch/making-submit/`, imports proved `EllIntrinsic`): the TODO.md prover bundle, sent AFTER the user's doc corrections landed (5 commits: Thm 9 removed, Bunni remap removed, Def 12 coordinate line, Γ_φ + L_{(1/2,0)} sweeps, Prop 6 restated, Defs 34–37 minted: π^f, π^log, ν utilization, c the level; call|put removed everywhere — 14 sites). **S1/S2** φ^σ = Q_M − (L_σ − Q_X/2)² closed kit (CONSTANT Γ_φ = −2, intrinsic 4p^{3/2}) + the boxed φ^ν→φ_{(χ,ε)} embedding EXPECTED REFUTED (homogeneity); **A1–A3** bid/ask: spread identity with the SIGN under suspicion (labels may be inverted on f∈(0,1) — instructed to record `bidask_labels_inverted`, not swap), retention-multiplicativity 1−(a⊗b)=(1−a)(1−b) (the machine answer to the which-algebra question), ask-composition multiplicative YES / ⊗_φ-compatible EXPECTED REFUTED; **R1/R2** the ramp band + step pair (Heaviside half of the pending Dirac reformulation — the δ-pair is NOT claimed). EXCLUDED and owed: the π^φ partials claim (TODO item 2 last display) — needs a function-of-flow definition of π^φ the doc doesn't have (Def 24's is a portfolio object); user ruling needed. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | usual gates; on return the Dirac reformulation (Rule 4's 𝕀, Convention 3 — user D4 sequencing ruling) unblocks and lands WITH the bid/ask verdicts. |
