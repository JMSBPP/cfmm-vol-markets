...
| `68d1b02a-8a3e-4a48-8f94-0b561b70e658` | `aristotle-geom` | 2026-08-10 | **6 sorry'd targets in `PayoffGeometry.lean`** (`scratch/geom-submit/`, imports the proved `EllIntrinsic`): the INTENT-TO-FORMALIZE bundle promoting four asserted Propositions. **P1/P2** → doc **Proposition 13** (the envelope relation `𝒟_p[π] = Q_X`, which the doc marks UNFORMALIZED, then `Γ = −½ \bar L_{(χ,ε)} p_φ^{−3/2}` — the claim being that Γ has NO free proportionality constant); **P3/P4** → doc **Proposition 7** (elasticity at balanced reserves `(1−ε_{X/M})/(1−χ_{X/M})`, then `κ_φ = (1−ε)/(2−ε)` under Definition 14's benchmark normalization, CPMM at 1/2 — hand-verified against Def 14 before submission); **P5** → doc **Proposition 12** (the ladder question: can a GEOMETRIC ladder realize the intrinsic-liquidity field? expected NO off the Cobb–Douglas slice, since a sum of two distinct powers is not a power — a refutation here is the direct warrant for Phase 15.2). Priority P1>P2>P5>P3>P4. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | **Do not integrate blind**: usual gates. On return, promote the settled Propositions to Theorems IN PLACE (renumbering is load-bearing — Prop 7/12/13 are cited by number) and record any refutation in-statement. |

| `4d696a77-aa60-4ef1-8da4-248a507d9921` | `aristotle-cohesion` | 2026-08-10 | **6 sorry'd targets in `CanonicalParam.lean`** (`scratch/cohesion-submit/`, imports the proved `EllIntrinsic`): the LEVEL SET as the transition channel between reserve and price coordinates — RTW's Canonical Parametrization (arXiv:2603.01344v1 §2.2) on our family, DIFFERENTIAL form only (the integral form's boundary conditions fail off the Cobb–Douglas slice; deliberate). **C1a/C1b** `dx/dp = −ℓ/(2p^{3/2})`, `dy/dp = ℓ/(2√p)`; **C2a/C2b** cohesion round trip (`dy/dx = −p` tangent-slope + `φ(x, yOf x) = c` on-curve); **C3** `L(p) = ℓ/(2p^{3/2}) = −Γ` + price impact `dp = −dx/L`; **C4** the pushforward — the field factors through (level, price) via `ellP` (hand-derived, ONE numeric check at (1/2,1/2,2,1) ≈ 3.48682; refute-and-correct explicitly invited on its exponent bookkeeping). Doc side on return: new statements in # TRADING_REGION consuming Definition 25's existing `(Q_X^L(p_φ), Q_M^L(p_φ))` — no new notation. Lit sweep verdicts already recorded: their `L(q)` IS `−Γ` (G1), their §2.3 Carr–Madan strip is P4's continuous half. Radicand guard carried throughout. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | **Do not integrate blind**: usual gates; C1a's Γ-conjunct overlaps geom P2 (`68d1b02a`) BY DESIGN (self-containedness) — on double return keep ONE carrier in-tree and cross-cite, do not land both texts. |
