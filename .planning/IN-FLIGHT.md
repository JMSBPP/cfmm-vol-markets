...
| `9786b137-f7e7-4175-af83-738c330b4022` | `aristotle-ell` | 2026-08-10 | **8 sorry'd targets in `EllIntrinsic.lean`** (`scratch/ell-submit/`): the intrinsic liquidity `ℓ_(ρ,ε)` of Risk–Tung–Wang arXiv:2603.01344v1 §2.1 (prereq Tung–Wang arXiv:2412.18580 App. B), both PDFs vendored to `plank/refs/cfmm/`. **L1 THE TARGET** `ellAt_eq_ell` — closed form `ℓ = 2√(ε(1−ε))(xy)^((ρ+1)/2) / ((1−ρ)(εx^ρ+(1−ε)y^ρ))`, hand-derived and numerically checked at 6 parameter points; **L7(a,b)** the HALF-KERNEL REDUCTION (`halfKernel_price_impact`, `halfKernel_osculates`) — the constant-product pool with `L ← ℓ` matches a general member's price AND first-order price impact, i.e. why `(sqrtPriceX96, L per tick)` suffices on-chain; **L4** state-constancy of `ℓ/√(xy)` holds ONLY at ρ=0 (refute-and-correct clause); L2/L3 the geometric-mean and CPMM limits; L5 degree-1 homogeneity; L6 the ρ→1⁻ pole (linear member = infinite depth). Priority L1>L7>L4>L2>L3>L5>L6. | the project returns (OUT_OF_BUDGET ⟹ `continue` on the SAME project) | **Do not integrate blind**: usual gates. On success this is the spine of milestone **M-A**: it makes Rule 5's `χ ← 1/2` a CHOICE rather than a silent dimensional precondition, and supplies the geometric object the Bunni-v2 LDF parametrizes (so LDF port sequences AFTER it). Doc glyph ruling: index by the characterizing parameters (`ℓ_(χ,ε)`), never a family-name subscript. |
