...
| **#2** Control target = **optimal set-point** `τ*_MEV`, not a feedback law over `e^σ` | User decision; the set-point is what the source derivation actually solves for | ⚠️ **Amended 2026-08-09** — still not a feedback law over `e^σ` (replication stays a feasibility region), but the `NEC-*` category adds a loop closing on the **FOC residual** `∂π̂^σ/∂τ_MEV`. **Correction, same day:** an earlier wording of this row said the loop reaches the set point *"without `Ḡ`"*. That is false — the residual is affine in `Ḡ` (see #15) — and `NEC-08` separately notes the loop does not enforce `Definition 36`'s equality constraint |
...
| **#10** `Δt` exogenous or endogenous | Cannot be settled in the doc layer; it is an identification question about a chosen venue | ⏸ **DEFERRED to Phase 6b** (`EST-02`), adjudicated by the structural-econometrics discipline |
| **#11** **Where on-chain the `∂ν/∂λ_MEV` feedback is implemented** | Open, not previously recorded — the site cannot be chosen before it is known whether the uniform-clearing precondition holds, whether `λ_MEV` and `ν` are readable per swap, and whether the accumulator write belongs in the swap path or a keeper. **The phrasing presumes an object whose existence `NEC-01`/`NEC-02` decide** | ⏸ **DEFERRED to Phase 6a** (`NEC-05`, `NEC-07`) |
| **#12** The **on-chain route runs before the estimation** | User ruling 2026-08-09, made on the premise that it removed a single point of failure. **That premise was refuted the same day (#15)** — it does not. The ordering is **retained on different grounds**: Phase 6a's precondition checks (`NEC-00`, `NEC-01`) terminate cheaply, and `NEC-04`'s coupling verdict can narrow what `EST-03` tests | ⚠️ Retained, rationale replaced |
| **#13** **Codebase** = Algebra Integral; **chain** = TBD | User ruling 2026-08-09. Our `φ` is a port of Algebra's `AdaptiveFee`, so `Φ`'s functional form and the volatility-oracle object are structurally the same rather than analogous. **Scope corrected same day:** this settles the *codebase*, not the *chain*. `Δt` is **chain-level**, so pool selection buys zero instrument variation and the `AdaptiveFee` argument says nothing about the instrument. Chain is selected on **measured** `Δt` dispersion (`LIT-04`, `EST-02`). Second risk now registered as `EST-08`: `Δt ⟂̸ σ`, since missed slots cluster with volatility and `σ` enters `φ` | ⚠️ Partially settled — chain open |
| **#15** **The "controller without `Ḡ`" premise is REFUTED** | Both review-gate reviewers independently composed `Theorem 30` + `Theorem 29` + `∂ν/∂τ = Ḡ·(∂λ/∂τ)` to get `∂π̂^σ/∂τ = (∂π̂^σ/∂φ)·[(1−φ_M)(1−φ_X) + (∂φ/∂ν)·Ḡ·(∂λ/∂τ)]`. `Ḡ` multiplies one of two bracket terms, so the residual is affine in it and evaluating the residual **is** evaluating `Ḡ`. Verified before acceptance. Consequences: `EST-04`'s demotion **withdrawn**; the sign/magnitude split restated (direction is `sign(∂²π̂^σ/∂τ²)`, stability needs a **bound** on `\|Ḡ\|`, and `H1` scales the loop gain); Phase 6a re-scoped to on-chain fixed-point iteration | ✓ **Settled — refuted**, pending `NEC-00`'s formal carrier |
| **#14** Research source classes = internal re-read + arXiv + non-arXiv on-chain material | User selection 2026-08-09. Dynamic-fee **natural experiments** were offered as a fourth class and **declined**; recorded as a decision, so reopening it is a scope change | — Pending |

> **Decision numbering.** The unnumbered rows above are `#1`…`#9` in table order; `#10`
> onward are labelled explicitly. `#10` was referenced by number in `ROADMAP.md` before it
> had a row here — this table is where it now lives.

---
*Last updated: 2026-08-09 — Phase 6 split into 6a/6b; Decisions #10–#15 recorded; the free-option premise REFUTED at the review gate (#15) and Phase 6a re-scoped; `EST-04`'s demotion withdrawn*
