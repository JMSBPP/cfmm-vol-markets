...
| O3 | ~~`phi_X` carries `nu`-dependence (`DOC` Definition 18) so `Rule 13`'s signature may be incomplete~~ **CLOSED AT SOURCE 2026-08-09.** `Rule 13` now reads `Phi(Theta_phi; sigma(i(t)), nu(t))` at `SRC:103 @ cf386de` -- it carries `nu`, so the signature is complete and the incompleteness concern is discharged by the source itself, not by this project. **The identification sense survives and is NOT closed:** `phi_X` being a function of the outcome is a bad-control hazard for any estimating equation, which is `EST-06`'s subject. **Any plan text asserting `O3 -- OPEN` or "`Rule 13` is therefore INCOMPLETE" is now a false statement about the source.** | ~~NOT-02, NOT-04~~, **EST-06** (bad control) only. **NEC-03** inherits the `beforeSwap` ordering question, which was never O3's identification content |
| O4 | `sigma` versus `sigma^2` units. **LOCUS MOVED 2026-08-09:** no longer `DOC`-vs-`SRC` but **internal to `SRC`** -- `Rule 13` (:103) carries `sigma(i(t))` while `Definition 32`'s `u_ex` (:62) carries `sigma^2(i(t))`. One document, two conventions. Still OPEN | NOT-05, EST-03, **NEC-02** (the accumulator reads a volatility-oracle quantity whose units must be pinned before it is summed), **LIT-04** (the dimensional check against Algebra's oracle). *Routing corrected 2026-08-09 -- this row pointed at `NEC-01` before the category was renumbered; `NEC-01` is now the uniform-clearing precondition.* |
