...
**Plans**: TBD — 7 plans expected, structured around the design's own gate:
...
whichever verdict returned. **What changed on 2026-08-09:** the gate no longer decides whether
a *controller* exists, only whether a *calibrated magnitude* does — provided Phase 6a returned
constructible on both `λ_MEV` and `ν`. If it did not, this gate recovers its original force and
the roadmap records that reversion rather than carrying the softened reading forward.
