...
**Success Criteria** (what must be TRUE):
  ...
  5. The output contract is delivered and **back-propagated into the Lean corpus**: `(a, b, c, d)` with covariance, the first-stage F, and the **admissible band** where `Ḡ` is bounded away from zero, intersected with `Theorem36`'s responsive band. Stage 1's verdict is recorded against `H2_dnu_dlamMEV_pos` in **both** `MevTaxControl.lean` and `MevTaxProgram.lean` as discharged or refuted; if refuted, the consequence is propagated rather than absorbed — **and stated precisely**: `Theorem34_opposed_signs` (`MevTaxProgram.lean:434`) does **not** "flip". Its binders are `(hdphi : 0 < dphidnu) (hdnu : dnudtau < 0)` — **no `hH1`, no `hH2`** — so refuting `H2` falsifies `hdnu` and the theorem becomes **inapplicable, its hypothesis unmet**, which is a different and weaker consequence than a reversal. The declaration that reaches `H2` by name is `Theorem34_signs_from_H1_H2` (:487, binder `hH2` at :489); it is that theorem's conclusion that no longer follows. The corrected law's sign reading changes accordingly. *(Corrected 2026-08-09 — the earlier wording conflated the two declarations, which is the `arb_add_fee_eq_lvr` failure class this project has already ruled against.)* Open item **O2** is restated here: if the estimation calibrates toward a `τ` assumed to minimise exposure, that assumption is load-bearing and unproved.
**Plans**: 7 plans, 7 waves (sequential; `parallelization: false`) — written 2026-08-09, on disk at `.planning/phases/06B-research-venue-and-estimating-mev/`. The four discipline requirements added at the 2026-08-09 review gate land in `06B-03`, **with** the pre-registration and never after data is examined. **Six of the seven carry `autonomous: false`** — only `06B-00` runs unattended: the phase carries a user checkpoint at the venue pick, a ratification before the freeze, the `EST-03` gate, and a terminal-verdict branch in every plan after the sweep. Each plan after `06B-00` also begins by reading a machine-readable `PHASE-6B-TERMINAL:` marker off disk and exits `0` reporting `NOT RUN` on a terminal branch, so the phase reaches `06B-06` on **every** path as this section's gate semantics require.
- [ ] `06B-00-PLAN.md` — `LIT-01` + `LIT-02` + `LIT-03`: the three-source research sweep, fourteen internal PDFs on a six-field schema, the arXiv sweep with its adversarial referee, non-arXiv material tagged lower-rigor, closing with the **instrument-selection rule written before any dispersion is measured** (**runs first; nothing downstream is specified before it returns**)
- [ ] `06B-01-PLAN.md` — `LIT-04` + `EST-01`: `ν` and `λ_ARB` derived in Algebra Integral's own state variables, the chain-vs-pool axis distinction, `ν`'s read path and its terminal constructibility verdict, and the candidate sets + window A/B boundary pre-declared and ratified (**blocking; has a checkpoint**)
- [ ] `06B-02-PLAN.md` — `EST-02`: `Δt` dispersion **measured on window A and reported as a number**, Decision #10 adjudicated under the structural-econometrics discipline, candidates ranked — **THE USER PICKS THE VENUE** (has a checkpoint)
- [ ] `06B-03-PLAN.md` — `EST-06` + `EST-07` + `EST-08` + `EST-09`: **THE FREEZE.** Bad-control resolution for `φ_X`, numeric thresholds with chain-time clustering, the `Δt ⟂̸ σ` pre-committed validity test, and the mandatory split-sample. Committed; its sha is the downstream pin (has a checkpoint)
- [ ] `06B-04-PLAN.md` — `EST-03`: **Stage 1 sign test** on window B — first-stage F reported before any second-stage output, three terminal outcomes ⟹ **HARD GATE** (has a checkpoint)
- [ ] `06B-05-PLAN.md` — `EST-04`: **Stage 2 magnitude** — a blocking gate check runs first and the plan **produces no file at all** unless `06B-04` recorded `VERDICT: GATE OPENS`
- [ ] `06B-06-PLAN.md` — `EST-05`: output contract on whichever branch returned, admissible band ∩ `Theorem36`'s responsive band, and **comment-only back-propagation into both Lean bundles** against `H2_dnu_dlamMEV_pos` (runs on every branch; has a checkpoint)
...
| 6b. Research, Venue, and Estimating `Ḡ` | 0/7 | Planned — 7 plans on disk, 7 waves; fully load-bearing (the demotion was withdrawn) | - |
