...
| implied maturity | `t★ = 2·ΔQ_v/N_σ` | **UNIT: seconds, PROVISIONAL** — σ² is on a per-second basis so N_σ shares it; CONFIRMATION owned by cfmm-lean4-spec issue #1 (the maturity-bridge formalization); the lens service publishes the unit it serves [BLOCKED-on #1 for the final pin] |
