...
\emph{Formalized:} \texttt{Panoptic.volOptionPayoff}; \texttt{volOptionPayoff\_nonneg}; \texttt{deltaQv\_of\_payoff}.
...
\emph{Formalized:} \texttt{Panoptic.latticeTheta} (the lattice quotient); \texttt{thetaAtm}.
...
\emph{Formalized (ATM case):} \texttt{theta\_atm\_closed\_form} --- \(\Theta_{ATM} = k\sigma/\sqrt{8\pi\tau}\). \textbf{General form OPEN} --- an Aristotle target (lattice → closed form).
...
\emph{Formalized:} \texttt{Panoptic.streamingPremium} (the discrete accumulation \(\sum_j \theta_j\,\Delta t\)); \texttt{streamingPremium\_succ}.
...
\emph{Formalized:} \texttt{Upsilon.upsilon} (\(\upsilon[p_\ell] = (p_\ell(\sigma^2{+}\Delta s) - p_\ell(\sigma^2))/\Delta s\)).
...
\emph{Formalized:} \texttt{Upsilon.upsilon\_volOption}; \texttt{upsilon\_eq\_deltaShares\_slot}; at the endogenous maturity \(\upsilon = T^\star/2\) (\texttt{variancePortfolio\_upsilon\_at\_tStar}, \texttt{tStar\_unit\_upsilon}).
...
\emph{Formalized:} \texttt{VolInstrument.logPortfolio}; \texttt{variancePortfolio} (\(= \text{logPortfolio} + \sigma^2(i(t))\, t/2\)); \texttt{logPortfolio\_nonneg}; \texttt{logPortfolio\_atm}.
...
\emph{Formalized (sensitivity half):} \texttt{variancePortfolio\_upsilon} (\(\upsilon = t/2\), \(p_{(\eta,\Delta_i)}\)-independent).
...
\emph{Formalized (weight law):} \texttt{GeomProfile.varswapWeight\_geometric}; \texttt{logContractLiquidity\_geometric}; \texttt{VolInstrument.strikeWeight\_bridge} (\(\xi^{\star} = \lambda^{-\Delta_i/2}\)).
...
\emph{Formalized:} \texttt{GeomProfile.geomWeight}.
...
\emph{Formalized:} \texttt{GeomProfile.geomWeight\_sum}; \texttt{geomWeight\_pos}; \texttt{geomWeight\_tendsto\_uniform}.
...
\tightlist
\item
  \(\lambda\) --- the \textbf{tick base}. \emph{Value:} \(\lambda = 1.0001\) (\href{../refs/uniswap-v3-core.pdf}{UNI\_V3}). \emph{Purpose:} the base of the price grid (Definition 8); every grid ratio in the document --- \(\lambda^{-\Delta_i}\) (Theorem 4), \(\xi^{\star} = \lambda^{-\Delta_i/2}\) (Proposition 6, \(\Theta_{\ell}\)) --- is a power of it. \emph{Economic meaning:} one tick = one basis point of price --- the minimal price quantum of the underlying market. \emph{Formalized:} \texttt{PosSpec.lam} (\texttt{lam\_pos}); hardcoded as \texttt{1.0001} in the \texttt{GeomProfile} carriers.
\end{itemize}
...
\emph{Formalized:} \texttt{VolInstrument.variancePortfolio\_unit\_upsilon}.
...
\emph{Formalized} (\texttt{EndogenousMaturity.lean}; \(N_\sigma \neq 0\)): bijection \texttt{dQvStarOfMaturity\_tStar} / \texttt{tStar\_dQvStarOfMaturity} / \texttt{maturity\_equivalence}; vega-exactness \texttt{tStar\_variancePortfolio\_upsilon}, \texttt{tStar\_unit\_upsilon}; \texttt{tStar\_pos}; \texttt{tStar\_strictMono\_dQvStar}; \texttt{tStar\_strictAnti\_Nσ}.
...
\emph{Formalized:} the leg encoding lives in \texttt{PanopticTokenId.plk} (\texttt{isLong}, \texttt{tokenType}); the doc-side ledger statement is \textbf{UNFORMALIZED} --- no Lean carrier states it yet.
...
\emph{Formalized:} \texttt{VolInstrument.priceEta}; \texttt{priceEta\_pos} (positivity, unconditional); \texttt{priceEta\_strictMono} (under \(\eta\,\Delta_i > 0\)); \texttt{priceEta\_one} (\(\eta = 1\) recovers \texttt{PosSpec.tickPrice}).
...
\emph{Formalized:} \texttt{CFMM.Eta.p\_eta\_partition\_change}; \texttt{exists\_partition\_change}; \texttt{p\_eta\_partition\_change\_int} (\texttt{exp/EtaPartitionChange}); Int24 split \texttt{eta\_split\_kernel\_identity} (\texttt{exp/eta}); rescaling \texttt{p\_eta\_eq\_P\_half\_rescaled}; update rule \texttt{p\_eta\_post\_eq} (\texttt{exp/EtaReplication} --- the plank-implemented half); convention bridge \texttt{EtaCurvature.priceEta\_eq\_p\_eta\_half}, \texttt{priceEta\_eq\_P\_half} (T28'a).
...
\emph{Formalized:} \texttt{GeomProfile.varswapWeight\_geometric}; \texttt{varswapWeight\_normalized}.
...
\emph{Formalized (sampling half):} \texttt{GeomProfile.logContractLiquidity\_geometric}; bridge to the ladder weights: \texttt{VolInstrument.strikeWeight\_bridge}.
...
\emph{Formalized:} \texttt{VolInstrument.deltaQM}, \texttt{deltaQX} (the defining displays).
...
\emph{Formalized:} \texttt{VolInstrument.deltaQM\_nonneg}; \texttt{deltaQX\_nonneg}; \texttt{deltaQM\_token0}.
...
\emph{Formalized:} \texttt{VolInstrument.cumulativeQM\_monotone}; \texttt{cumulativeQX\_monotone}; \texttt{cumulativeQM\_const}; \texttt{cumulativeQX\_const}; \texttt{exists\_least\_reaching}.
...
\emph{Formalized:} \texttt{PhiCES.phiCES} --- \textbf{opposite leg orientation; the FLAG below is OPEN}.
...
\emph{Formalized:} \texttt{PhiCES.phiCES} (12/12 axiom-clean): \texttt{phiCES\_tendsto\_phiEps}; \texttt{phiCES\_one} (\(\epsilon_{X/M} = 1\) linear); \texttt{phiCES\_zero\_half\_eq\_geom}; \texttt{phiCES\_homogeneous}/\allowbreak\texttt{\_pos}/\allowbreak\texttt{\_mono}. \emph{Narrowed, declared:} \texttt{phiCES\_concave} is RADIAL concavity only --- joint concavity in \((Q_X,Q_M)\) is OPEN.
...
\emph{Formalized:} the definitional layer is \textbf{UNFORMALIZED} --- the in-tree \texttt{CurvatureTwo.curvTwo} is the closed form of Proposition 7 by fiat, not this definition.
...
\emph{Formalized:} \texttt{CurvatureTwo.curvTwo}; \texttt{curvTwo\_linear\_zero}; \texttt{curvTwo\_pos\_of\_lt\_one}; \texttt{curvTwo\_strictAnti\_rho}; \texttt{curvTwo\_mem\_Ico}; \texttt{rhoOfCurv} (both round trips); \(\bar\epsilon_{X/M}\) = \texttt{subElast} (\texttt{subElast\_zero}, \texttt{subElast\_tendsto\_one}).
...
\emph{Formalized:} \texttt{EtaTilde.curvOfTilde} / \texttt{EtaCurvature.curvIndex}. \emph{(Lean names predate the doc symbols and are NOT renamed --- standing doc-glyph/Lean-name split.)}
...
\emph{Formalized:} \texttt{curvOfTilde\_not\_curvature} (the machine-checked discriminating witness); \(\varsigma_{X/M}\) factors through the share alone: \texttt{curvIndex\_is\_rho\_zero\_slice}.
...
\emph{Formalized:} \texttt{phiCES\_zero\_half\_eq\_geom} (the \((\epsilon_{X/M} \to 0,\ \chi_{X/M} = 1/2)\) member is the geometric mean).
...
\emph{Formalized} (\texttt{EtaTilde}, 23/23 axiom-clean; doc \(\chi_{X/M}\) ↔ Lean \texttt{etaTilde}): anchor \texttt{etaTilde\_ratio}; observable \texttt{etaTilde\_eq\_priceEta\_step}; bijection \texttt{etaTilde\_mem\_Ioo}, \texttt{etaTilde\_strictMono}, \texttt{etaOfTilde\_etaTilde}, \texttt{etaTilde\_etaOfTilde}, \texttt{etaTilde\_half\_iff}, \texttt{etaTilde\_tendsto\_atTop}/\allowbreak\texttt{\_atBot}; bridge to \(\varsigma_{X/M}\): \texttt{curvIndex\_eq\_of\_etaTilde}, \texttt{curvOfTilde\_etaTilde}, \texttt{tildeOfCurv\_curvOfTilde}; range \texttt{curvOfTilde\_mem\_Ioo} (the \(t \in (0,1)\) hypothesis is NECESSARY --- \texttt{Real.rpow} is \(\log|x|\)-based outside it).
...
\emph{Formalized:} \texttt{admissible\_iff}; \texttt{zero\_curv\_iff}.
...
\emph{Formalized:} \texttt{curvOfTilde\_strictMono} (the true direction); \textbf{REFUTED:} \texttt{not\_curvOfTilde\_strictAnti} --- machine-checked negation of the antitone reading, witness above. \emph{(Retitled from the in-doc ``Lemma (Curvature--Share Monotonicity)'': ``Lemma'' is not a taxonomy class, and per Theorem 9 \(\varsigma_{X/M}\) is share asymmetry, not curvature.)}
...
\emph{Formalized:} \texttt{Panoptic.streamingPremium}; \texttt{streamingPremium\_succ} (\(\Sigma_{N+1} = \Sigma_N + \theta_N\Delta t\)).
...
\emph{Formalized} (\texttt{EndogenousMaturity.lean} --- the floor is the GREATEST admissible exposure):
...
\emph{Formalized:} \texttt{tStarJointMult}: \texttt{\_nonneg} (on \(T^{\star}(t) \geq 0\)), \texttt{\_antitone} (\(\downarrow \sigma^2_R\)), \texttt{\_zero} (\(= T^{\star}(t)\) at \(\sigma^2_R = 0\)), \texttt{\_exhausted} (\(= 0\) at \(\sigma^2_R = \sigma^2_K\)). \textbf{Alternates formalized and REJECTED} (the rejection record is first-class): \(T^{\star}_{\text{sub}}\) (\texttt{joint\_candidates\_disagree} --- off-domain floor placement only), \(T^{\star}_{\text{quad}}\) (\((1-r^2) \geq (1-r)\): pro-holder under vol clustering).
...
\emph{Formalized:} \texttt{VolInstrument.probOr\_eq}; \texttt{probOr\_comm}; \texttt{probOr\_assoc}; \texttt{probOr\_zero}; \texttt{probOr\_mem\_Icc}; \texttt{probOr\_mono}; \texttt{probOr\_hazard}.
...
\emph{Formalized:} \texttt{FlairOptimization.flairMulti\_affine}; \texttt{W\_j\_lt\_W}; \texttt{flairMulti\_le\_corner}; \texttt{flairMulti\_corner\_attained\_levels}; \texttt{flairMulti\_saturation\_limit}; \texttt{flairMulti\_strict\_below\_saturation}; \texttt{Theta\_lambda\_identification}.
...
\emph{Formalized} (\texttt{MevOptimization.lean}): \texttt{ptrade\_mem\_Ioc}; \texttt{ptrade\_eq\_one\_iff}; \texttt{ptrade\_strictAntiOn} (on \([0,\infty)\)); \texttt{ptrade\_monotoneOn\_dt}; \texttt{ptrade\_monotoneOn\_sigma}; \texttt{ptrade\_strictConvexOn} (+ \texttt{\_convexOn}); \texttt{ptrade\_tendsto\_atTop}.
...
--- the ONLY object carrying the guard \(\sigma^2(i(t))\,\Delta t < 8\); reuse this symbol downstream. \emph{Formalized:} \(\lambda_{\text{ARB}} \geq 0\): \texttt{mevMulti\_nonneg}, CPMM weight \texttt{mevWeight\_cpmm\_pos} (\(\cdot\Delta t\) carried). Tier (ii) is \textbf{UNFORMALIZED/OPEN} (T19 omitted --- no carrier).
...
\emph{Formalized:} \texttt{mevMulti\_anti\_phibar}; \texttt{mevMulti\_anti\_alpha}; \texttt{mevMulti\_anti\_u}; \texttt{mevMulti\_mono\_beta}.
...
\emph{Formalized:} \texttt{mevMulti\_ge\_corner}; \texttt{mevMulti\_corner\_attained\_levels}; \texttt{mevMulti\_saturation\_limit} {[}CORRECTED: Aristotle-added \(0 \leq \bar\phi_{\max} + u_{\max}\alpha_{\max}\) --- the \(\mathbb{P}_{\Delta_{\text{ARB}}}\) pole{]}; \texttt{mevMulti\_strict\_above\_saturation}; \texttt{mevMulti\_exists\_min\_compact} {[}CORRECTED: fees \(\geq 0\) required --- unbounded below on arbitrary compact \(\Theta\){]}; packaged \texttt{Theta\_lambdaMEV\_identification}, \texttt{mevMulti\_min\_gt\_corner} (at \(u = u_{\max}\)).
...
\emph{Formalized:} budget half \texttt{flair\_budget\_pins\_mean\_fee}, \texttt{flair\_budget\_mean}; path carriers \texttt{flairPath}/\allowbreak\texttt{mevPath} with bridges \texttt{flairPath\_schedule}, \texttt{mevPath\_schedule}, \texttt{flairPath\_sum}, \texttt{flairPath\_budget\_mean}; the constant-\(\sigma\) display at PATH level \texttt{mev\_ge\_flat\_under\_flair\_budget\_const\_sigma}, strict \texttt{mev\_gt\_flat\_under\_flair\_budget\_const\_sigma}; the refutation \texttt{mev\_ge\_flat\_under\_flair\_budget\_false}.
...
\emph{Formalized} (\texttt{MevJointProgram.lean}): \texttt{mevTotal} (plain addition, with the \(\otimes_{\phi}\) correspondence as its own lemma \texttt{mevTotal\_probOr\_hazard}); \texttt{mevTotal\_eq\_arb\_of\_sandwich\_zero}; \texttt{mevTotal\_mevMulti\_eq\_of\_sandwich\_zero}; cadence \texttt{mev\_mono\_dt}. \emph{(The removed recycling mechanism's carriers --- the \texttt{mevNet} family, \texttt{taxFraction} --- remain in-tree as formalized-not-adopted.)}
...
\emph{Formalized} (\texttt{TauMevAlgebra}, 14/14 axiom-clean --- the carriers parked at Rule 7 now attached): (A) \texttt{tau\_monoid\_mem}; \texttt{tau\_monoid\_ge}/\allowbreak\texttt{\_gt}; \texttt{tau\_intensity\_effect(\_strict)}; \texttt{tau\_no\_targeting}; \texttt{tau\_hazard\_exact}; (B) \texttt{tau\_split\_budget}; \texttt{tau\_split\_intensity\_neutral}; \texttt{tau\_split\_flair\_linear}; \texttt{tau\_split\_mevNet\_bridge}; (D) \texttt{tau\_scaling\_not\_monoid\_hom}; \texttt{tau\_order\_matters}; \texttt{tau\_split\_breaks\_hazard}.
\end{document}
