...
# Forge -vvvv output from the pair-test job only (isolated from foundry install noise).
ci-plank-test-lens run_id job_id="":
    #!/usr/bin/env bash
    set -euo pipefail
    run="{{run_id}}"
    job="{{job_id}}"
    if [[ -z "$job" ]]; then
      job=$(gh run view "$run" --json jobs --jq '.jobs[] | select(.name=="pair-test") | .databaseId')
    fi
    gh run view "$run" --log --job "$job" \
      | sed -n 's/^pair-test\tpair-test\t[0-9TZ:.\+-]*Z //p' \
      | { grep -vE '^##\[(group|endgroup)\]|^\^|^(shell|env):|^[[:space:]]' || true; }

ci-plank-test-lens-latest:
    #!/usr/bin/env bash
    set -euo pipefail
    run=$(gh run list --workflow=tmp_push.yml --branch="$(git branch --show-current)" --limit 1 --json databaseId --jq '.[0].databaseId')
    exec just ci-plank-test-lens "$run"
